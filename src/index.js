...
api.getUserInfo()
.then((res)=>{
    userProfile.setUserInfo({username:res.name, userjob:res.about, useravatar:res.avatar, userId:res._id})
})
...
const createCard = (data) => {
    const elementCard = new Card({data, modalCard, userId:userProfile._userId, handleSubmitLike:(isLiked)=> {
        if(!isLiked){
...
const editProfileAvatar = new PopupWithForm("#profile__img-edit",handleEditAvatar);
...
function handleProfileFormSubmit(data) {
    ...
    .then((res)=>{
        userProfile.setUserInfo({username:res.name, userjob: res.about, useravatar:res.avatar})
        editPopup.close();
    })
